...
const handleLogin = async () => {
  console.group('🚀 === HANDLE LOGIN APPELÉ ===')
  ...
  // Validation
  console.log('📋 Appel de validateForm...')
  if (!validateForm()) {
  ...
  try {
    console.log('📤 Appel authStore.login...')
    console.log('authStore existe?', !!authStore)
    console.log('authStore.login existe?', typeof authStore.login)
    ...
    if (result.success) {
      ...
      // Attendre un peu
      console.log('⏱️ Attente 500ms...')
      await new Promise((resolve) => setTimeout(resolve, 500))

      // Vérifier l'état
      console.log('🔍 Vérification post-login:')
      console.log('- isAuthenticated:', authStore.isAuthenticated)
      console.log('- user:', authStore.user)
      console.log('- token in storage:', localStorage.getItem('auth_token') ? 'EXISTS' : 'NULL')
      console.log('- user in storage:', localStorage.getItem('user') ? 'EXISTS' : 'NULL')

      // Si pas authentifié, forcer
      if (!authStore.isAuthenticated) {
        console.warn('⚠️ isAuthenticated est false, tentative de forçage...')

        const token = localStorage.getItem('auth_token')
        const userStr = localStorage.getItem('user')

        if (token && userStr) {
          console.log('✅ Forçage avec données du storage...')
          const user = JSON.parse(userStr)
          authStore.setAuthData(user)
        } else {
          console.error('❌ Pas de données dans storage!')
        }
      }
...
// Auto-focus
onMounted(() => {
  console.log('🎬 Login.vue monté')
  console.log('authStore disponible:', !!authStore)
  console.log('authStore.login disponible:', typeof authStore.login)
...
<template>
  <!-- Le template reste identique, juste le script change -->
  <div
