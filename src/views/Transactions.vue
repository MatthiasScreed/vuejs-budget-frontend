<template>
  <div class="transactions-page">
    <!-- Debug Panel (à activer si besoin) -->
        ...
        >
          <div class="pending-item-content">
            ...
            <div class="pending-item-actions">
              ...
              <select
                @change="handleQuickCategorize(tx, $event)"
            ...
            <div class="transaction-desktop">
              ...
              <div class="col-span-2">
                <div class="desktop-actions">
                  <button @click="handleEdit(tx)" class="icon-btn" title="Modifier">
                    ✏️
                  </button>
                  <button @click="handleDuplicate(tx)" class="icon-btn" title="Dupliquer">
                    📋
                  </button>
                  <button @click="handleDelete(tx)" class="icon-btn" title="Supprimer">
                    🗑️
                  </button>
                </div>
              </div>
      ...
      >
        <div class="modal-content">
          <div class="modal-header">
            <h2>{{ isEditing ? 'Modifier' : 'Nouvelle' }} transaction</h2>
            <button @click="handleCloseModal" class="modal-close">✕</button>
          </div>
          <div class="modal-body">
            <p class="modal-info">
              {{ isEditing ? `Édition de "${editingItem?.description}"` : 'Créer une nouvelle transaction' }}
            </p>
            <p class="modal-placeholder">
              📝 Formulaire de transaction à implémenter<br>
...
// ✅ CORRECTION : Utiliser directement les computed au lieu de storeToRefs
...
const debugMode = ref(false) // Mettre à true pour debug
...
// ✅ Stats du mois avec CORRECTION complète
const stats = computed(() => {
  console.log('📊 [STATS] Début calcul')
  console.log('📦 [STATS] Transactions:', displayedTransactions.value?.length || 0)

  // Sécurité : Vérifier que transactions existe
  if (!displayedTransactions.value || displayedTransactions.value.length === 0) {
    console.log('⚠️ [STATS] Pas de transactions, retour stats vides')
    return {
      totalIncome: 0,
      totalExpenses: 0,
      balance: 0,
  ...
  console.log('📅 [STATS] Transactions ce mois:', thisMonth.length)

  // ✅ FORCER la conversion en nombre avec Number()
  const income = thisMonth
    .filter(t => t.type === 'income')
    .reduce((sum, t) => {
      const amount = Number(t.amount) || 0
      console.log(`  💰 [STATS] Income: ${t.description} = ${amount}€`)
      return sum + amount
    }, 0)
  ...
  const expenses = thisMonth
    ...
    .reduce((sum, t) => {
      const amount = Number(t.amount) || 0
      console.log(`  💸 [STATS] Expense: ${t.description} = ${amount}€`)
      return sum + amount
    }, 0)

  const balance = income - expenses

  console.log('✅ [STATS] Totaux - Income:', income, 'Expenses:', expenses, 'Balance:', balance)
...
const currentPageNumber = computed(() => {
  return pagination.value?.current_page || localFilters.value.page || 1
})
...
const canGoNext = computed(() => {
  return currentPageNumber.value < totalPages.value
})
